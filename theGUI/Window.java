...
//import theGUI.Clickable;
//import theGUI.Component;
//import theGUI.Drawable;
//import theGUI.Hoverable;
//import theGUI.Interactive;
...
//        GraphicsEnvironment.getLocalGraphicsEnvironment();
    ...
    public double getStandardScale() {
		return hasSecondMonitor() ? 330/500.0 : 1.0;}
	
	private boolean hasSecondMonitor() {
    ...
    public void drawImageCentered(String path, double x, double y) {
        ...
        withGraphics(g -> g.drawImage(img,
                getTranslateInstance(toNative(x) - img.getWidth() / 2, toNative(y) - img.getHeight() / 2), null));
    }
    ...
    private BufferedImage ensureLoaded(String imagePath) throws IllegalStateException {
        if (!images.containsKey(imagePath)) {
            try {
                ...
                if (pixelScale == 1) {
                    ...
                    AffineTransformOp op = new AffineTransformOp(getScaleInstance(pixelScale, pixelScale),
    ...
    public List<String> getPressedKeys() {
        return pressedSnapshot.stream().filter(i -> i instanceof KeyInput).map(i -> ((KeyInput) i).key)
                .collect(toList());
    }

    public List<String> getTypedKeys() {
        return releasedSnapshot.stream().filter(i -> i instanceof KeyInput).map(i -> ((KeyInput) i).key)
                .collect(toList());
    }
